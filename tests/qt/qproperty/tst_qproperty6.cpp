...
** Copyright (C) 2022 Andreas Reischuck
...
{
    W_OBJECT(tst_QProperty)
private slots:
#define DECLARE_TEST(NAME) void NAME(); W_SLOT(NAME)
    DECLARE_TEST(functorBinding);
    DECLARE_TEST(basicDependencies);
    DECLARE_TEST(multipleDependencies);
    DECLARE_TEST(bindingWithDeletedDependency);
    DECLARE_TEST(dependencyChangeDuringDestruction);
    DECLARE_TEST(recursiveDependency);
    DECLARE_TEST(bindingAfterUse);
    DECLARE_TEST(bindingFunctionDtorCalled);
    DECLARE_TEST(switchBinding);
    DECLARE_TEST(avoidDependencyAllocationAfterFirstEval);
    DECLARE_TEST(boolProperty);
    DECLARE_TEST(takeBinding);
    DECLARE_TEST(stickyBinding);
    DECLARE_TEST(replaceBinding);
    DECLARE_TEST(changeHandler);
    DECLARE_TEST(propertyChangeHandlerApi);
    DECLARE_TEST(subscribe);
    DECLARE_TEST(changeHandlerThroughBindings);
    DECLARE_TEST(dontTriggerDependenciesIfUnchangedValue);
    DECLARE_TEST(bindingSourceLocation);
    DECLARE_TEST(bindingError);
    DECLARE_TEST(bindingLoop);
    DECLARE_TEST(realloc);
    DECLARE_TEST(changePropertyFromWithinChangeHandler);
    DECLARE_TEST(changePropertyFromWithinChangeHandlerThroughDependency);
    DECLARE_TEST(changePropertyFromWithinChangeHandler2);
    DECLARE_TEST(settingPropertyValueDoesRemoveBinding);
    DECLARE_TEST(genericPropertyBinding);
    DECLARE_TEST(genericPropertyBindingBool);
    DECLARE_TEST(setBindingFunctor);
    DECLARE_TEST(multipleObservers);
    DECLARE_TEST(arrowAndStarOperator);
    DECLARE_TEST(notifiedProperty);
    DECLARE_TEST(typeNoOperatorEqual);
    DECLARE_TEST(bindingValueReplacement);
    DECLARE_TEST(quntypedBindableApi);
    DECLARE_TEST(readonlyConstQBindable);
    DECLARE_TEST(qobjectBindableManualNotify);
    DECLARE_TEST(qobjectBindableSignalTakingNewValue);

    DECLARE_TEST(testNewStuff);
    DECLARE_TEST(qobjectObservers);
    DECLARE_TEST(compatBindings);
    DECLARE_TEST(metaProperty);

    DECLARE_TEST(modifyObserverListWhileIterating);
    DECLARE_TEST(compatPropertyNoDobuleNotification);
    DECLARE_TEST(compatPropertySignals);

    DECLARE_TEST(noFakeDependencies);

    DECLARE_TEST(bindablePropertyWithInitialization);
    DECLARE_TEST(noDoubleNotification);
    DECLARE_TEST(groupedNotifications);
    DECLARE_TEST(groupedNotificationConsistency);
    DECLARE_TEST(uninstalledBindingDoesNotEvaluate);

    DECLARE_TEST(notify);

    DECLARE_TEST(bindableInterfaceOfCompatPropertyUsesSetter);
...
{
    W_OBJECT(ChangeDuringDtorTester)
...
public:
    void fooChanged(int newFoo) W_SIGNAL(fooChanged, newFoo);
    void barChanged() W_SIGNAL(barChanged);
    void compatChanged() W_SIGNAL(compatChanged);

public slots:
    void fooHasChanged() { fooChangedCount++; }
    W_SLOT(fooHasChanged);
    void barHasChanged() { barChangedCount++; }
    W_SLOT(barHasChanged);
    void compatHasChanged() { compatChangedCount++; }
    W_SLOT(compatHasChanged);
...
public:
    ...
    QBindable<int> bindableFoo() const { return QBindable<int>(&fooData); }
...
{
    W_OBJECT(FakeDependencyCreator)

public:
    void prop1Changed() W_SIGNAL(prop1Changed);
    void prop2Changed() W_SIGNAL(prop2Changed);
    void prop3Changed() W_SIGNAL(prop3Changed);
...
{
    W_OBJECT(PropertyWithInitializationTester)
public:
    void prop1Changed() W_SIGNAL(prop1Changed);
