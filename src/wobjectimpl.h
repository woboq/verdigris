...
struct concatenate_helper<std::index_sequence<I1...>, std::index_sequence<I2...>>
{
...
constexpr StaticString<1> concatenate(StaticStringList<>)
{
    return {""};
}
template <typename T>
constexpr auto concatenate(StaticStringList<binary::Leaf<T>> s)
{
    return s.root.data;
}
template <typename A, typename B>
constexpr auto concatenate(StaticStringList<binary::Node<A, B>> s)
{
...
struct IntermediateState
{
    ...
    {
        auto s2 = binary::tree_append(strings, s);
        return IntermediateState<decltype(s2), Ints..., Flag | Strings::size>{s2};
    }
...
constexpr bool getSignalIndexHelperCompare(T1, T2)
{
    return false;
}
template <typename T>
constexpr bool getSignalIndexHelperCompare(T f1, T f2)
{
    return f1 == f2;
}
...
private:
    ...
    static constexpr std::enable_if_t<
        getSignalIndexHelperCompare(binary::get<SigIdx>(signalState).func, property.notify) || true, int>
    helper(int)
    {
...
template <int NameLength, typename Methods, typename Constructors, typename Properties, typename Enums,
          typename ClassInfos, typename Interfaces, int SignalCount>
struct ObjectInfo
{
...
static constexpr auto makeObjectInfo(StaticStringArray<N> &name)
{
    ...
    return ObjectInfo<N, decltype(methodInfo), decltype(constructorInfo), decltype(propertyInfo), decltype(enumInfo),
                      decltype(classInfo), decltype(interfaceInfo), sigCount>{
...
struct isPublic<T, M, decltype(T::w_GetAccessSpecifierHelper(std::declval<M>()))> : std::true_type
{
};
template <typename T, typename M, typename = void>
...
template <typename T, typename M>
struct isProtected<T, M, decltype(Derived<T>::test(std::declval<M>()))> : std::true_type
{
};
    ...
    static constexpr uint adjustFlags(uint f, M)
    {
        if (!(f & (W_Access::Protected.value | W_Access::Private.value | W_Access::Public.value))) {
            ...
            f |= isPublic<T, M>::value ? W_Access::Public.value
                                       : isProtected<T, M>::value ? W_Access::Protected.value : W_Access::Private.value;
        }
...
template <typename T>
struct MetaTypeIdIsBuiltIn<T, typename std::enable_if<QMetaTypeId2<T>::IsBuiltIn>::type> : std::true_type
{
};
...
struct PropertyGenerator
{
    ...
    static constexpr auto generate(State s, Prop prop)
    {
        ...
        constexpr uint moreFlags =
            (QtPrivate::IsQEnumHelper<typename Prop::PropertyType>::Value ? uint(PropertyFlags::EnumOrFlag) : 0);
        constexpr uint defaultFlags = 0 | PropertyFlags::Stored | PropertyFlags::Scriptable | PropertyFlags::Designable;
...
private:
    ...
    process(State s, Func,
            std::enable_if_t<std::is_same<T, typename QtPrivate::FunctionPointer<Func>::Object>::value, int> = 0)
    {
...
template <typename T>
struct NotifySignalGenerator<T, false>
{
    template <typename>
    static constexpr int offset()
    {
    ...
    template <int, typename State, typename Prop>
    static constexpr auto generate(State s, Prop)
    {
        return s;
    }
};
...
struct EnumGenerator
{
    ...
    {
        return s.addString(e.name)
            .template add< // name
                Enum::flags, Enum::count, DataIndex>();
    }
};
...
struct HandleArgsHelper<A, Args...>
{
    ...
    static constexpr auto result(const Strings &ss, const ParamTypes &paramTypes)
    {
        ...
        // This way, the overload of result will not pick the StaticString one if it is a tuple (because registered
        // types have the priority)
...
struct HandleArgNames
{
    ...
    template <typename Strings>
    static constexpr auto result(const Strings &ss, StaticStringList<> pn)
    {
        return HandleArgNames<N - 1>::result(ss.template add<1>(), pn);
    }
};
template <>
struct HandleArgNames<0>
{
    template <typename Strings, typename PN>
    static constexpr auto result(const Strings &ss, PN)
    {
        return ss;
    }
};
...
constexpr auto generateDataArray(const ObjI &objectInfo)
{
    ...
    constexpr int constructorParamIndex =
        paramIndex + paramOffset<decltype(objectInfo.methods)>(make_index_sequence<ObjI::methodCount>{});
 ...
 */
template <typename S, typename I, typename O, typename N, typename T>
struct BuildStringDataHelper;
template <std::size_t... S, std::size_t... I, std::size_t... O, std::size_t... N, typename T>
struct BuildStringDataHelper<std::index_sequence<S...>, std::index_sequence<I...>, std::index_sequence<O...>,
...
const qt_meta_stringdata_t<sizeof...(I), sizeof...(S)>
    BuildStringDataHelper<std::index_sequence<S...>, std::index_sequence<I...>, std::index_sequence<O...>,
                          std::index_sequence<N...>, T>::qt_meta_stringdata = {
        {Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(N - 1, stringdata_offset + O
...
struct ComputeOffsets<H, T...>
{
    template <std::size_t... I>
    static std::index_sequence<0, (I + H)...> func(std::index_sequence<I...>);
...
{
    return BuildStringDataHelper<make_index_sequence<sums(N...)>, make_index_sequence<sizeof...(N)>,
                                 typename ComputeOffsets<N...>::Result, std::index_sequence<N...>,
                                 T>::qt_meta_stringdata.data;
}
template <typename T, std::size_t... I>
 ...
 */
template <typename I>
struct build_int_data;
template <std::size_t... I>
struct build_int_data<std::index_sequence<I...>>
{
    static const uint data[sizeof...(I)];
};
template <std::size_t... I>
const uint build_int_data<std::index_sequence<I...>>::data[sizeof...(I)] = {uint(I)...};

/** Returns the QMetaObject* of the base type. Use SFINAE to only return it if it exists */
template <typename T>
static constexpr auto parentMetaObject(int) W_RETURN(&T::W_BaseType::staticMetaObject) template <typename T>
...
inline auto propSet(F f, O *o, const T &t) W_RETURN(((o->*f)(t), 0)) template <typename F, typename O, typename T>
inline auto propSet(F f, O *o, const T &t) W_RETURN(o->*f = t) template <typename O, typename T>
inline void propSet(std::nullptr_t, O *, const T &)
{
}

template <typename F, typename O, typename T>
inline auto propGet(F f, O *o, T &t) W_RETURN(t = (o->*f)()) template <typename F, typename O, typename T>
inline auto propGet(F f, O *o, T &t) W_RETURN(t = o->*f) template <typename O, typename T>
inline void propGet(std::nullptr_t, O *, T &)
{
}

template <typename F, typename M, typename O>
inline auto propNotify(F f, M m, O *o) W_RETURN(((o->*f)(o->*m), 0)) template <typename F, typename M, typename O>
inline auto propNotify(F f, M, O *o) W_RETURN(((o->*f)(), 0)) template <typename... T>
inline void propNotify(T...)
{
}

template <typename F, typename O>
inline auto propReset(F f, O *o) W_RETURN(((o->*f)(), 0)) template <typename... T>
...
struct FriendHelper
{
    ...
    static void registerMethodArgumentType(int _id, void **_a)
    {
        if (_id == I) {
            ...
            *reinterpret_cast<int *>(_a[0]) = _t && arg < P::ArgumentCount ? _t[arg] : -1;
        }
    }
    ...
    static void qt_static_metacall_impl(QObject *_o, QMetaObject::Call _c, int _id, void **_a,
                                        std::index_sequence<MethI...>, std::index_sequence<ConsI...>,
                                        std::index_sequence<PropI...>)
    {
        Q_UNUSED(_id) Q_UNUSED(_o) Q_UNUSED(_a) if (_c == QMetaObject::InvokeMetaMethod)
        {
        ...
        else if (_c == QMetaObject::IndexOfMethod)
        {
            *reinterpret_cast<int *>(_a[0]) =
                sums((1 + indexOfMethod<T, MethI>(reinterpret_cast<void **>(_a[1])))...) - 1;
        }
        else if (_c == QMetaObject::CreateInstance) { nop((createInstance<T, ConsI>(_id, _a), 0)...); }
        else if ((_c >= QMetaObject::ReadProperty && _c <= QMetaObject::QueryPropertyUser)
                 || _c == QMetaObject::RegisterPropertyMetaType)
        {
    ...
    /** Same for W_GADGET */
    template <typename T, size_t... MethI, size_t... ConsI, size_t... PropI>
    static void qt_static_metacall_impl(T *_o, QMetaObject::Call _c, int _id, void **_a, std::index_sequence<MethI...>,
                                        std::index_sequence<ConsI...>, std::index_sequence<PropI...>)
    {
        Q_UNUSED(_id) Q_UNUSED(_o) Q_UNUSED(_a) if (_c == QMetaObject::InvokeMetaMethod)
        {
    ...
    static void *qt_metacast_impl(T *o, const char *_clname, std::index_sequence<InterfaceI...>)
    {
        ...
        if (_clname == QByteArray(sd))
            return o;
        void *result = nullptr;
        nop((interfaceMetaCast<decltype(binary::get<InterfaceI>(T::W_MetaObjectCreatorHelper::objectInfo.interfaces))>(
                 result, o, _clname),
             0)...);
...
template <typename T>
constexpr auto createMetaObject()
{
    return FriendHelper::createMetaObject<T>();
}
template <typename T, typename... Ts>
auto qt_metacall_impl(Ts &&... args)
{
    return FriendHelper::qt_metacall_impl<T>(std::forward<Ts>(args)...);
}
template <typename T>
auto qt_metacast_impl(T *o, const char *_clname)
{
    using ObjI = decltype(T::W_MetaObjectCreatorHelper::objectInfo);
    return FriendHelper::qt_metacast_impl<T>(o, _clname, make_index_sequence<ObjI::interfaceCount>{});
}
template <typename T, typename... Ts>
auto qt_static_metacall_impl(Ts &&... args)
{
    using ObjI = decltype(T::W_MetaObjectCreatorHelper::objectInfo);
    return FriendHelper::qt_static_metacall_impl<T>(std::forward<Ts>(args)..., make_index_sequence<ObjI::methodCount>{},
...
#define W_MACRO_TEMPLATE_STUFF(...)                                                                                    \
    W_MACRO_CONCAT(W_MACRO_TEMPLATE_STUFF_HELPER,                                                                      \
...
#define W_OBJECT_IMPL_COMMON(...)                                                                                      \
    ...
    {                                                                                                                  \
        static constexpr auto objectInfo =                                                                             \
            w_internal::makeObjectInfo<W_MACRO_FIRST_REMOVEPAREN(__VA_ARGS__)::W_ThisType>(                            \
                W_MACRO_STRIGNIFY(W_MACRO_FIRST_REMOVEPAREN(__VA_ARGS__)));                                            \
        static constexpr auto data =                                                                                   \
            w_internal::generateDataArray<W_MACRO_FIRST_REMOVEPAREN(__VA_ARGS__)::W_ThisType>(objectInfo);             \
        static constexpr auto string_data = data.first;                                                                \
        static constexpr auto int_data = data.second;                                                                  \
    };                                                                                                                 \
    W_MACRO_TEMPLATE_STUFF(__VA_ARGS__)                                                                                \
    W_STATICMETAOBJECT_CONSTEXPR const QMetaObject W_MACRO_FIRST_REMOVEPAREN(__VA_ARGS__)::staticMetaObject =          \
...
#define W_OBJECT_IMPL(...)                                                                                             \
    ...
    W_MACRO_TEMPLATE_STUFF(__VA_ARGS__)                                                                                \
    void W_MACRO_FIRST_REMOVEPAREN(__VA_ARGS__)::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id,        \
                                                                    void **_a)                                         \
    {                                                                                                                  \
...
#define W_GADGET_IMPL(...)                                                                                             \
    ...
    void W_MACRO_FIRST_REMOVEPAREN(__VA_ARGS__)::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id,        \
                                                                    void **_a)                                         \
    {                                                                                                                  \
        w_internal::qt_static_metacall_impl<W_MACRO_FIRST_REMOVEPAREN(__VA_ARGS__)>(                                   \
            reinterpret_cast<W_MACRO_FIRST_REMOVEPAREN(__VA_ARGS__) *>(_o), _c, _id, _a);                              \
    }

#define W_NAMESPACE_IMPL(...) W_OBJECT_IMPL_COMMON(__VA_ARGS__)
